...
export function DndProvider({ children }: { children: React.ReactNode }) {
  ...
  const collisionDetection: CollisionDetection = useCallback(
    (args) => {
      if (activeDragTypeRef.current === "document" || activeDragTypeRef.current === "tracking_sheet" || activeDragTypeRef.current === "signature_sheet" || activeDragTypeRef.current === "intercalaire") {
  ...
  return (
    <DndRegistryContext.Provider value={registryValue}>
      ...
      >
        {children}
        {/* DragOverlay désactivé temporairement pour test */}
      </DndContext>
