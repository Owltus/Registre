...
export function DeleteDocumentDialog({ doc, onClose, onConfirm }: DeleteDocumentDialogProps) {
  return (
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[92vw] max-w-sm border bg-background shadow-lg rounded-lg flex flex-col overflow-hidden focus:outline-none">
          ...
          <div className="px-6 py-4">
            <p className="text-sm text-muted-foreground">
              Voulez-vous vraiment supprimer <span className="font-medium text-foreground">{doc?.title || "Sans titre"}</span> ? Cette action est irréversible.
            </p>
          </div>
