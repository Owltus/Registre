...
import type { TrackingSheet } from "./types"

interface DeleteTrackingSheetDialogProps {
  sheet: TrackingSheet | null
  onClose: () => void
  onConfirm: () => void
}

export function DeleteTrackingSheetDialog({ sheet, onClose, onConfirm }: DeleteTrackingSheetDialogProps) {
  return (
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[92vw] max-w-sm border bg-background shadow-lg rounded-lg flex flex-col overflow-hidden focus:outline-none">
          ...
          <div className="px-6 py-4">
            <p className="text-sm text-muted-foreground">
              Voulez-vous vraiment supprimer <span className="font-medium text-foreground">{sheet?.title || "Sans titre"}</span> ? Cette action est irréversible.
            </p>
          </div>
          ...
          <Dialog.Description className="sr-only">
            Confirmer la suppression de la feuille de suivi
          </Dialog.Description>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
